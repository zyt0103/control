...
% 	[X, Y] = meshgrid(First_Delay_Id, First_Freq_Id);
% 	figure;surf(X / os, Y*37.5, abs(First_Mat.'));
% 	xlabel('时延/bit');ylabel('频偏/Hz');shading interp;
% 	xlim([0 50]);ylim([-1000 1000]);
%     figure;surf(First_Delay_Id, First_Freq_Id,abs(First_Mat.'))
% 	xlabel('频域','fontsize',30,'horizontalalignment','center')
% 	ylabel('时域','fontsize',30,'horizontalalignment','center')
% 	title('粗同步时频联合相关峰值图','fontsize',30,'horizontalalignment','center')
	% 细同步
...
% 		xlabel('频域','fontsize',30,'horizontalalignment','center')
% 		ylabel('时域','fontsize',30,'horizontalalignment','center')
% 		title('细同步时频联合相关峰值图','fontsize',30,'horizontalalignment','center')
